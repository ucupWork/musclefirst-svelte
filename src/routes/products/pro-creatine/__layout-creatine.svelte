...
<script lang="ts">
    ...
    let productName: string = "Pro Creatine";
	let ready: boolean;
    let seconds: number = 0;
    let showNotification: boolean = false;
	let imageNotifiaction: boolean = true;
    let productLink: string = 'https://shopee.co.id/product/8802988/289998377';
...
<SEO 
    ogImage={'https://musclefirst-space.sgp1.cdn.digitaloceanspaces.com/Musclefirst/product/new-product/pro-creatine/pronew-06_grknvm.png'}
    title={`Creatine Terbaik untuk Menambah Massa Otot - ${productName}`} 
...
<div class="relative flex flex-col justify-center items-center w-full h-auto overflow-auto scroll-smooth scrollbar-thin scrollbar-thumb-zinc-900 scrollbar-track-bg-transparent bg-white">
    ...
    {#if showNotification}
        <div class="fixed left-4 top-16 w-80 md:w-80 h-auto flex md:hidden flex-row justify-center items-center bg-transparent z-[90]">
            <Notification 
                ...
                {imageNotifiaction}
                imgLink={'https://musclefirst-space.sgp1.cdn.digitaloceanspaces.com/Musclefirst/product/new-product/pro-creatine/pronew-06_grknvm.png'}
                productName="Pro Creatine" 
    ...
    <div class="fixed inset-x-0 bottom-0 w-full h-auto hidden md:flex flex-col md:flex-row justify-between items-center bg-zinc-900 z-[90] px-12 md:px-24">
        ...
        {/if}
        <ButtonOrder classes="bg-gradient-to-r from-amber-500 to-orange-500 shadow-lg border shadow-amber-500/50" linkHref={'/products/pro-creatine'} />
    </div>
    <!-- End Footer -->
</div>