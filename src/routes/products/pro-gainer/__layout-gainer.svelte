...
<script lang="ts">
    ...
    let productName: string = "Pro Gainer";
	let ready: boolean;
    let seconds: number = 0;
    let showNotification: boolean = false;
	let imageNotifiaction: boolean = true;
    let productLink: string = 'https://shopee.co.id/product/8802988/896956194';
...
<SEO 
    ogImage={'https://musclefirst-space.sgp1.cdn.digitaloceanspaces.com/Musclefirst/product/new-product/pro-gainer/pronew-02_xs8ik9.png'}
    title={`Susu Protein Terbaik Untuk Bulking - ${productName}`} 
    description={`${productName} merupakan susu untuk menggemukan badan dan menambah massa otot dengan memenuhi surplus kalori harian. Wujudkan badan ideal kalian dengan Gainer Terbaik.`}
    schemas={[
        websiteSchema, 
        organizationSchema
    ]}  
...
<div class="relative flex flex-col justify-center items-center w-full h-auto overflow-auto scroll-smooth scrollbar-thin scrollbar-thumb-zinc-900 scrollbar-track-bg-transparent bg-white">
    ...
    {#if showNotification}
        <div class="fixed left-4 top-16 w-80 md:w-80 h-auto flex md:hidden flex-row justify-center items-center bg-transparent z-[90]">
            <Notification 
                ...
                {imageNotifiaction}
                imgLink={'https://musclefirst-space.sgp1.cdn.digitaloceanspaces.com/Musclefirst/product/new-product/pro-gainer/pronew-02_xs8ik9.png'}
                productName="Pro Gainer" 
    ...
    <div class="fixed inset-x-0 bottom-0 w-full h-auto hidden md:flex flex-col md:flex-row justify-between items-center bg-zinc-900 z-[90] px-12 md:px-24">
        <!-- YYYY-MM-DD H:m:s -->
        {#if $lpCountdown}
        <CountTime bgColor="zinc-900" flexType="flex-row" textColor="white" width="16" height="12" textSize="xl" borderColor="yellow" dateFormatText="2022-07-31 24:00:00" endTimeText="The time has come!" />
        {/if}      
        <ButtonOrder classes="bg-gradient-to-r from-amber-500 to-orange-500 shadow-lg border shadow-amber-500/50" linkHref={'/products/pro-gainer'} />
    </div>
    <!-- End Footer -->
</div>