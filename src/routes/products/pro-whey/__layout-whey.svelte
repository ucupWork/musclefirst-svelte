...
<script lang="ts">
    ...
    let productName: string = "Pro Whey";
	let ready: boolean;
    let seconds: number = 0;
    let showNotification: boolean = false;
	let imageNotifiaction: boolean = true;
    let productLink: string = 'https://shopee.co.id/Muscle-First-Pro-Whey-100-2lbs-Whey-Protein-900gr-25-Serving-i.8802988.5876940790?sp_atk=302acf6b-2a3b-4e5e-95b9-f2f7d0e19eba&xptdk=302acf6b-2a3b-4e5e-95b9-f2f7d0e19eba';
...
<SEO 
    ogImage={'https://musclefirst-space.sgp1.cdn.digitaloceanspaces.com/Musclefirst/product/new-product/pro-whey/pronew-03_tjnsg9.png'}
    title={`Whey Protein Harga Ekonomis - Halal - BPOM - Labdoor - ${productName}`} 
    description={`${productName} Pro Whey merupakan susu whey protein dengan harga ekonomis yang mengandung 24gr Protein dan juga bersertifikat Halal, BPOM dan Sudah Uji Labdoor.`}
    schemas={[
        websiteSchema, 
        organizationSchema
    ]}  
...
<div class="relative flex flex-col justify-center items-center w-full h-auto overflow-auto scroll-smooth scrollbar-thin scrollbar-thumb-zinc-900 scrollbar-track-bg-transparent bg-white">
    ...
    {#if showNotification}
        <div class="fixed left-4 top-16 w-80 md:w-80 h-auto flex md:hidden flex-row justify-center items-center bg-transparent z-[90]">
            <Notification 
                ...
                {imageNotifiaction}
                imgLink={'https://musclefirst-space.sgp1.cdn.digitaloceanspaces.com/Musclefirst/product/new-product/pro-whey/pronew-03_tjnsg9.png'}
                productName="Pro Whey" 
    ...
    <div class="fixed inset-x-0 bottom-0 w-full h-auto hidden md:flex flex-col md:flex-row justify-between items-center bg-zinc-900 z-[90] px-12 md:px-24">
        ...
        {/if}      
        <ButtonOrder classes="bg-gradient-to-r from-amber-500 to-orange-500 shadow-lg border shadow-amber-500/50" linkHref="/products/pro-whey" />
    </div>
    <!-- End Footer -->
</div>