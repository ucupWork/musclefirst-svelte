...
import type { QueryDatabaseResponse, GetPagePropertyResponse } from "@notionhq/client/build/src/api-endpoints";
import type { BlogFrontmatter } from "$typings/blog.type";
import type { CareerFrontmatter } from "$typings/career.type";

type NotionProperty = GetPagePropertyResponse["type"];
type BlogProperty = keyof BlogFrontmatter;
type CareerProperty = keyof CareerFrontmatter;
	...
	switch (propertyType) {
		case "rich_text": // Array<RichTextItemResponse>
			...
			if (propertyValue.length > 1) {
				...
				propertyValue.forEach((value: { plain_text: string; }) => {
		...
		case "multi_select": // Array<SelectPropertyResponse>
			...
			return propertyValue.map((select: { name: any; color: any; }) => {
				return {
					name: select.name,
					color: select.color,
				};
...
export function getCareerProperty(property: CareerProperty, notionPageRes: NotionPageResponse) {
