<script lang="ts">
    import Loader from '$shared/pixi/Loader.svelte'
      ...
      if (container) {
        ...
        stars = new Array(starAmount).fill(null).map(() => {
          const star = new PIXI.Sprite(
            PIXI.Texture.from('/assets/site/star.png')
          ) as Star
  ...
  <div class="wrapper bg-[#1c2127] h-auto">
    <Application bind:instance={app}>
      <Loader resources={['/assets/site/star.png']}>
        <Ticker on:tick={tick} />
        <ParticleContainer
