...
function readNumber(iter) {
  ...
  while (!iter.end()) {
    ...
    temp += curr;
    if (hasDot && is$1.dot(next))
      break;
    if (is$1.dot(curr))
      hasDot = true;
    if (is$1.dots(next, next2))
      break;
...
function scan(source, options = {}) {
  ...
  while (iter.next()) {
    ...
    if (is$1.comment(curr, next)) {
      ...
      if (quoteStack.length && is$1.escape(lastToken2.value)) {
        tokens.pop();
...
function make_tag_function(fn) {
  ...
  return (input, ...vars) => {
    let string = make_array$1(input).reduce((s, c, i) => s + c + get_value2(vars[i]), "");
    return fn(string);
  };
}
function next_id() {
  ...
  while (iter.next()) {
    ...
    if (isBlock(tokenType) && isBlockBreak) {
      if (!next && rules.length && !curr.isSymbol("}")) {
        let last2 = rules[rules.length - 1].value;
      ...
      let expand2 = props.length > 1 && groupdValue.length === props.length;
...
function generate$2(token, last2) {
  let result = "";
  if (token.type === "block") {
    ...
    let skip = token.origin && last2 === token.origin.name.join(",");
...
function removeParens(tokens) {
  ...
  while (head && head.isSymbol("(") && last2 && last2.isSymbol(")")) {
    tokens = tokens.slice(1, tokens.length - 1);
    head = tokens[0];
    last2 = tokens[tokens.length - 1];
  }
  return tokens;
}
...
function composeStyle(block) {
  const style = block.value.map((n) => n.type === "block" ? composeStyle(n) : composeStyleRule(n.name, n.value)).join("");
...
function by_unit(fn) {
  return (...args) => {
    let units2 = [], values = [];
    for (let arg of args) {
      let { unit: unit3, value } = parse$4(arg);
      if (unit3 !== void 0) {
        units2.push(unit3);
      }
      if (value !== void 0) {
...
function parse$3(input) {
  ...
  while (iter.next()) {
    ...
    if (curr.isSymbol(":") && !name) {
      ...
      commands2[name] = transformNegative(name, joinTokens(tokens), negative);
      tokens = [];
      name = null;
      negative = false;
    } else if (!curr.isSymbol(";")) {
...
function create_shape_points(props, { min, max }) {
  ...
  let { unit: unit2, value } = parse$4(pr);
...
const Uniforms = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  uniform_time,
  uniform_mousex,
  uniform_mousey,
...
const Expose = add_alias({
  ...
  P({ context, pick, position }) {
    ...
    return expand((...args) => {
      ...
      let last2 = stack ? stack.last(1) : "";
      if (normal) {
        if (!context[counter]) {
          context[counter] = {};
        }
        last2 = context[counter].last_pick;
      }
      if (args.length > 1) {
        let i = args.findIndex((n) => n === last2);
        if (i !== -1) {
          args.splice(i, 1);
        }
      }
  ...
  filter: lazy((upstream, ...args) => {
    ...
    if (values.every((n) => /^[\d.]/.test(n) || /^(\w+)/.test(n) && !/[{}<>]/.test(n))) {
      let { frequency, scale: scale2 = 1, octave, seed = upstream.seed, blur, erode, dilate } = get_named_arguments(values, [
        "frequency",
        "scale",
        "octave",
        "seed",
...
class Rules {
  ...
  read_var(value, coords) {
    ...
    if (group[value] !== void 0) {
      ...
      if (result[0] == "(") {
        let last2 = result[result.length - 1];
        if (last2 === ")") {
  ...
  class Doodle extends HTMLElement {
    ...
    register_uniform_mouse(uniforms) {
      if (!this.uniform_mouse_callback) {
        let { uniform_mousex: uniform_mousex2, uniform_mousey: uniform_mousey2 } = Uniforms;
    ...
    register_uniform_resolution(uniforms) {
      if (!this.uniform_resolution_observer) {
        let { uniform_width: uniform_width2, uniform_height: uniform_height2 } = Uniforms;
        const setProperty = () => {
    ...
    register_uniform_time() {
      ...
      if (!this.is_uniform_time_registered) {
        let { uniform_time: uniform_time2 } = Uniforms;
        try {
